...
function convertImperialToMetric(text: string): string {
  ...
  const fractionalToDecimal = mixedToDecimal.replace(
    /(\d+\/\d+)/g,
    function (match) {
  ...
  const rangeToTo = fractionalToDecimal.replace(
    /(\d+\.?\d*)-(\d+\.?\d*)\s*oz/g,
    function (match, p1, p2) {
  ...
  const ounceToCl = rangeToTo.replace(
    /(\d+\.?\d*)\s*oz/g,
    function (match, p1) {
  ...
  const inchToCm = ounceToCl.replace(
    /(\d+\.?\d*)\s*inches?/g,
    function (match, p1) {
...
const RandomRecipe: React.FC = () => {
  ...
  for (let i = 1; i <= 15; i++) {
    ...
    if (ingredient || measure) {
      const convertedMeasure = measure
        ? convertImperialToMetric(measure)
        : measure;
      ingredientList.push(
        <li key={i}>
  ...
  const convertedInstructions = convertImperialToMetric(
  ...
  return (
    <StyledDrinkRecipe>
      ...
      <ConvertedInstructionsContainer>
